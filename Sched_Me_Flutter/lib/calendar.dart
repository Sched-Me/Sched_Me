...
class _Calendar extends State<Calendar> {
    ...
    return Consumer<CalendarModel>(
      builder: (context, calendarModel, child) {
        return SfCalendar(
          ...
          onTap: (CalendarTapDetails details) => calendarTapped(details, context, calendarModel),
  ...
  void calendarTapped(CalendarTapDetails calendarTapDetails, BuildContext context, CalendarModel calendarModel) {
      ...
      showDialog(
        ...
        builder: (BuildContext context) {
          return AlertDialog(
            title: Row(
              children: [
                Text(calendarModel.name),
                const Spacer(),
                IconButton(
                    onPressed: () {
                      setState(() {
                        calendarModel._appointments.removeAt(calendarModel._appointments.length-1);
                        calendarModel.classes.removeAt(calendarModel._appointments.length-1);
                      });
                    },
            ...
            content: Wrap(
              children: [
                Column(
                  ...
                  children: [
                    Text("Start Time: ${formatDate(calendarModel.startTime)}"),
                    Text("End Time: ${formatDate(calendarModel.endTime)}"),
                    Text("Notes: ${calendarModel.notes ?? ''}"),
                    Text("Location: ${calendarModel.location ?? ''}"),
                  ],
                ),
              ],
            ),
